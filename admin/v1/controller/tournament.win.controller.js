const { getlist, insertone, updateone, deleteone } = require('../helper/tournament.win.helper');
...
exports.getList = async (req, res, next) => {
    var response = await getlist(req.body);
    res.json(response);
};
...
exports.updateOne = async (req, res, next) => {
    ...
    if (response.status) {
        ...
        if (response.status == false) {
            ...
            var deletePath = path.join(__dirname, process.env.HORDINGPATH, response.data.winnerphoto);
            await fs.unlink(deletePath, (err) => { });
        }
        res.json(response);
    } else {
