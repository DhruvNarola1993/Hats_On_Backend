var Tournamentpanel = require('./../../../api/v1/models/tournament.win.model');
...
async function getAll(params) {
    try {
        ...
        var findAll;
        if (search != "") {
            countTotal = await Tournamentpanel.countDocuments({ "userid": { "$regex": search, "$options": 'i' } });
            findAll = await Tournamentpanel.aggregate([
                { $sort: sortObject },
                { $match: { "userid": { "$regex": search, "$options": 'i' } } },
        ...
        if (findAll.length > 0) {
            ...
            return {
                status: false,
                msg: "Tournament Win User is not present."
            };
        }
    } catch (error) {
        return {
...
async function getUpdateOne(params) {
    try {
        const { _id, updateby, imageurl, description, userid, winnerpoint } = params;
        var findOne = await Tournamentpanel.findByIdAndUpdate(_id, {
            $set: {
                winnerphoto: imageurl,
...
async function oneDelete(params) {
    try {
        ...
        if (deleteOne != null) {
            ...
            return {
                status: false,
                msg: "Tournament Win User is not delete."
            };
        }
    } catch (error) {
        return {
